"""
LOCAL-SOC-SLM Telemetry Writer (Stage 1)
Bounded, fail-open, append-only JSONL event stream.
"""
import os
import json
import time
...
# Default configuration (can be overridden in constructor for testing)
DEFAULT_BUFFER_ROOT = Path("/home/swiig/Documents/blueprint/overnight/.telemetry_buffer")
DEFAULT_ROTATE_BYTES = 1 * 1024 * 1024  # 1 MB
DEFAULT_MAX_BUFFER_BYTES = 50 * 1024 * 1024  # 50 MB

class TelemetryWriter:
    """
    Bounded, fail-open JSONL telemetry writer.
    
    Stage 2 Contract Note: The sync_telemetry.py process MUST tolerate 
    FileNotFoundError when reading from the outbox/ directory, as this 
    writer is permitted to delete outbox files to enforce the hard storage cap.
    """
    def __init__(self, buffer_root=None, rotate_bytes=None, max_buffer_bytes=None):
        ...
        if self.rotate_bytes >= self.max_buffer_bytes:
            raise ValueError("rotate_bytes must be strictly less than max_buffer_bytes")
    ...
    def _warn(self, err_type, msg, event=None):
        now = time.time()
        if now - self._warnings.get(err_type, 0) > 300:  # Rate limit: 1 per 5 mins
            self._warnings[err_type] = now
            try:
                ctx = ""
                if event:
                    rid = event.get('remediation_id', '')
                    tf = event.get('target_file', '')
                    if rid or tf:
                        ctx = f" [remediation_id={rid}, target={tf}]"
                print(f"[TELEMETRY WARNING] {msg}{ctx}", file=sys.stderr)
            except Exception:
                pass
    ...
    def _get_total_size(self):
        ...
        try:
            if self.current_file.exists():
                total += self.current_file.stat().st_size
    ...
    def _enforce_cap(self, event=None):
        try:
            ...
            while total > self.max_buffer_bytes:
                ...
                if outbox_files:
                    outbox_files[0].unlink(missing_ok=True)
                else:
                    if not self._drop_mode:
                        self._drop_mode = True
                        self._warn("cap_drop", "Telemetry cap exceeded with no eligible files to delete. Entering drop mode.", event)
                    break
                total = self._get_total_size()
    ...
    def log_attempt(self, event):
        ...
        GUARANTEE: This method will NEVER raise an exception to the caller.
        """
        try:
            # 1. Caller Immutability & Sanitization
            safe_event = dict(event)
            for bad_key in ['api_keys', 'prompt', 'generated_code', 'raw_response', 'env_vars']:
                safe_event.pop(bad_key, None)
                
            # Always try to enforce cap to allow recovery from drop mode
